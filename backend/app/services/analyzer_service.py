...
from app.services.formality_classifier import get_formality_classifier
...
class AnalyzerService:
    ...
    def analyze_sentences(
        ...
        # ML 격식 분류 + 일관성 체크
        formality_by_index: dict[int, dict] = {}
        clf = get_formality_classifier()
        if clf and sentence_tokens:
            clf_results = clf.classify_batch([s.text for s in sentence_tokens])
            formality_by_index = {s.index: r for s, r in zip(sentence_tokens, clf_results)}
        ...
        for r in pass1_sentences:
            ...
            fm = formality_by_index.get(idx, {})
            sentences.append(SentenceResult(
                index=idx,
                text=sentence_map.get(idx, ""),
                formality=fm.get("formality"),
                formality_ko=fm.get("formality_ko"),
                formality_confidence=fm.get("formality_confidence"),
                is_formality_consistent=fm.get("is_formality_consistent"),
